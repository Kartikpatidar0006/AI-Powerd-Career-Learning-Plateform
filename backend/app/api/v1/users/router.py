...
def complete_onboarding(
    ...
    if not prof_id and payload.profession_slug:
        p_stmt = select(Profession).where(Profession.slug == payload.profession_slug)
        prof_obj = db.execute(p_stmt).scalars().first()
        if prof_obj:
            prof_id = prof_obj.id
