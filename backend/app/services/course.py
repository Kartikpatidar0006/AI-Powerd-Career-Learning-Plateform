...
class CourseService:
    ...
    def _get_or_404(self, course_id: uuid.UUID) -> Course:
        """Load a course by ID or raise ``CourseError(NOT_FOUND)``.

        Args:
            course_id: UUID of the course to load.

        Returns:
            The ``Course`` ORM instance.

        Raises:
            CourseError: With code ``NOT_FOUND`` if no row matches.
        """
    ...
    def _assert_skill_exists(self, skill_id: uuid.UUID) -> None:
        """Raise ``CourseError(SKILL_NOT_FOUND)`` if the skill is missing.

        Args:
            skill_id: UUID of the skill to validate.

        Raises:
            CourseError: With code ``SKILL_NOT_FOUND`` if missing.
        """
        if self._skill_repo.get_by_id(skill_id) is None:
            raise CourseError(
                f"Skill with id '{skill_id}' was not found.",
                code=CourseError.SKILL_NOT_FOUND,
            )
