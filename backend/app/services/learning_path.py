...
class LearningPathService:
    ...
    def _get_or_404(self, learning_path_id: uuid.UUID) -> LearningPath:
        """Load a learning path entry by ID or raise ``LearningPathError(NOT_FOUND)``.

        Args:
            learning_path_id: UUID of the entry to load.

        Returns:
            The ``LearningPath`` ORM instance.

        Raises:
            LearningPathError: With code ``NOT_FOUND`` if no row matches.
        """
    ...
    def _assert_profession_exists(self, profession_id: uuid.UUID) -> None:
        """Raise if the profession is missing.

        Args:
            profession_id: UUID of the profession to validate.

        Raises:
            LearningPathError: With code ``PROFESSION_NOT_FOUND`` if missing.
        """
        if self._profession_repo.get_by_id(profession_id) is None:
            raise LearningPathError(
                f"Profession with id '{profession_id}' was not found.",
                code=LearningPathError.PROFESSION_NOT_FOUND,
            )

    def _assert_skill_exists_in_profession(
        self, skill_id: uuid.UUID, profession_id: uuid.UUID
    ) -> None:
        """Raise if the skill is missing or does not belong to the profession.

        This enforces the domain invariant that a learning path step can only
        reference a skill that has already been catalogued under the same
        profession.

        Args:
            skill_id: UUID of the skill to validate.
            profession_id: UUID of the profession the skill must belong to.

        Raises:
            LearningPathError: ``SKILL_NOT_FOUND`` if skill UUID doesn't exist.
            LearningPathError: ``SKILL_NOT_IN_PROFESSION`` if the skill exists
                but belongs to a different profession.
        """
        skill = self._skill_repo.get_by_id(skill_id)
        if skill is None:
            raise LearningPathError(
                f"Skill with id '{skill_id}' was not found.",
                code=LearningPathError.SKILL_NOT_FOUND,
            )
        if skill.profession_id != profession_id:
            raise LearningPathError(
                f"Skill '{skill_id}' does not belong to profession '{profession_id}'. "
                "Only skills catalogued under the same profession can be added "
                "to its learning path.",
                code=LearningPathError.SKILL_NOT_IN_PROFESSION,
            )
