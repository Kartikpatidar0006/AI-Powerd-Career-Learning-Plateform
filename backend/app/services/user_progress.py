...
class UserProgressService:
    ...
    def _get_or_404(self, progress_id: uuid.UUID) -> UserProgress:
        """Load a progress record by ID or raise ``UserProgressError(NOT_FOUND)``.

        Args:
            progress_id: UUID of the record to load.

        Returns:
            The ``UserProgress`` ORM instance.

        Raises:
            UserProgressError: With code ``NOT_FOUND`` if no row matches.
        """
    ...
    def _assert_user_exists(self, user_id: uuid.UUID) -> None:
        """Raise ``UserProgressError(USER_NOT_FOUND)`` if the user is missing.

        Validates via a direct session lookup rather than importing
        ``UserRepository`` to avoid heavy cross-repository coupling.

        Args:
            user_id: UUID of the user to validate.

        Raises:
            UserProgressError: With code ``USER_NOT_FOUND`` if missing.
        """
        ...
        if self._db.get(User, user_id) is None:
            raise UserProgressError(
                ...
                code=UserProgressError.USER_NOT_FOUND,
            )

    def _assert_skill_exists(self, skill_id: uuid.UUID) -> None:
        """Raise ``UserProgressError(SKILL_NOT_FOUND)`` if the skill is missing.

        Args:
            skill_id: UUID of the skill to validate.

        Raises:
            UserProgressError: With code ``SKILL_NOT_FOUND`` if missing.
        """
        if self._skill_repo.get_by_id(skill_id) is None:
            raise UserProgressError(
                f"Skill with id '{skill_id}' was not found.",
                code=UserProgressError.SKILL_NOT_FOUND,
            )
